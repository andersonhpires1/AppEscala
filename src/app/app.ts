...
export class App {
  ...
  public POSTOS_TRABALHO = [
    { id: 'posto_gru_1', name: 'Base Guarulhos (Vibra GRU-1)', latitude: -23.4356, longitude: -46.4731 },
    { id: 'posto_tiete_2', name: 'Posto Marginal Tietê (Vibra Tietê-2)', latitude: -23.5189, longitude: -46.6125 },
    { id: 'posto_dutra_3', name: 'Posto Rodovia Dutra (Vibra Dutra-3)', latitude: -23.4215, longitude: -46.4128 },
    { id: 'posto_abc_4', name: 'Posto ABC (Vibra ABC-4)', latitude: -23.6558, longitude: -46.5312 }
  ];

  public getCollabGeoData(collab: Collaborator) {
    const lat_res = (collab as any).latitude_residencia ?? (collab as any).latitudeResidencia;
    const lng_res = (collab as any).longitude_residencia ?? (collab as any).longitudeResidencia;
    const pt_id = (collab as any).posto_trabalho_id ?? (collab as any).postoTrabalhoId;

    if (lat_res && lng_res && pt_id) {
      const posto = this.POSTOS_TRABALHO.find(p => p.id === pt_id) || this.POSTOS_TRABALHO[0];
      return {
        latRes: Number(lat_res),
        lngRes: Number(lng_res),
        postoId: pt_id,
        posto
      };
    }

    // Deterministic fallback based on collab ID hash to ensure realism
    let hash = 0;
    for (let i = 0; i < collab.id.length; i++) {
      hash = collab.id.charCodeAt(i) + ((hash << 5) - hash);
    }
    
    const postoIdx = Math.abs(hash) % this.POSTOS_TRABALHO.length;
    const posto = this.POSTOS_TRABALHO[postoIdx];

    const latOffset = ((Math.abs(hash >> 2) % 200) - 100) / 1000; // -0.1 to +0.1 degrees
    const lngOffset = ((Math.abs(hash >> 4) % 200) - 100) / 1000; // -0.1 to +0.1 degrees
    
    const latRes = posto.latitude + latOffset + (hash > 0 ? 0.04 : -0.04);
    const lngRes = posto.longitude + lngOffset + (hash > 0 ? -0.04 : 0.04);
  ...
  public getICTForCollab(collab: Collaborator) {
    const geo = this.getCollabGeoData(collab);
    
    // Calculate direct and estimate route distance / duration
    const R = 6371;
    const lat1 = geo.latRes;
    const lng1 = geo.lngRes;
    const lat2 = geo.posto.latitude;
    const lng2 = geo.posto.longitude;
    ...
    const distanceKm = Number((directDistance * 1.35).toFixed(1));
    const baseDurationMin = Math.round((distanceKm / 35) * 60);

    // Turn info & fatigue factors
    const shiftInfo = this.getShiftHoursInfo(collab.shift);
    const seqStats = this.getConsecutiveWorkStats(collab);
    const isNightShift = collab.shift.toUpperCase().includes('NOITE');

    // Weather factors at start hour
    const rawWeather = this.rawHourlyWeather();
    let temp = 22;
    let humidity = 65;
    let rainProb = 0;
    let weatherCode = 0;
    ...
    if (rawWeather.length > 0) {
      const match = rawWeather.find(w => w.hour === shiftInfo.startHour) || rawWeather[0];
      temp = match.temp;
      humidity = match.humidity;
      rainProb = match.rainProb;
      weatherCode = match.weatherCode;
      windSpeed = match.wind;
    }

    // Deterministic traffic congestion based on hour and collaborator ID
    let hash = 0;
    for (let i = 0; i < collab.id.length; i++) {
      hash = collab.id.charCodeAt(i) + ((hash << 5) - hash);
    }
    const trafficSeed = Math.abs(hash) % 15;
    
    // Is start hour during peak hours?
    const isPeak = (shiftInfo.startHour >= 7 && shiftInfo.startHour <= 9) || (shiftInfo.startHour >= 17 && shiftInfo.startHour <= 19);
    let delayMin = isPeak ? 15 + trafficSeed : Math.round(trafficSeed / 2);
    
    // Rain adds extra delay
    if (rainProb > 50) {
      delayMin += 8;
    }

    const finalDurationMin = baseDurationMin + delayMin;

    // Heat Index
    let heatIndex = temp;
    if (temp >= 27) {
      heatIndex = Math.round(0.5 * (temp + 61.0 + ((temp - 68.0) * 1.2) + (humidity * 0.094)));
    }

    // Mathematical ICT score (0 to 100)
    let score = 100;
    const factors: string[] = [];

    // 1. Shift / Fatigue impact
    if (isNightShift) {
      score -= 10;
      factors.push('Trabalho Noturno (-10)');
    }
    if (seqStats.streak >= 4) {
      score -= 15;
      factors.push(`Acúmulo de Dias Consecutivos: ${seqStats.streak} dias (-15)`);
    }

    // 2. Commute duration / traffic impact
    if (finalDurationMin > 60) {
      score -= 20;
      factors.push('Tempo de Deslocamento >60 min (-20)');
    } else if (finalDurationMin > 35) {
      score -= 10;
      factors.push('Tempo de Deslocamento Moderado (-10)');
    }
    if (delayMin > 15) {
      score -= 15;
      factors.push(`Congestionamento Severo: +${delayMin} min (-15)`);
    }

    // 3. Weather impact
    if (rainProb > 60) {
      score -= 15;
      factors.push('Risco Alto de Chuva / Pistas Escorregadias (-15)');
    }
    if (heatIndex > 32) {
      score -= 15;
      factors.push(`Estresse Térmico Elevado: Sensação de ${heatIndex}°C (-15)`);
    } else if (temp < 15) {
      score -= 10;
      factors.push(`Frio Extremo no Trajeto: ${temp}°C (-10)`);
    }
    ...
    // Alerts
    const sstAlertText = heatIndex > 32 
      ? `Estresse térmico detectado (${heatIndex}°C). Hidrate-se e realize pausas preventivas!`
      : '';
    const weatherAlertText = rainProb > 50 
      ? `Chuva forte ou instabilidades previstas (${rainProb}%). Redobre a atenção na direção!`
      : '';
    const trafficAlertText = delayMin > 15 
      ? `Congestionamento severo no trajeto (+${delayMin} min). Sugere-se flexibilização ou rota alternativa!`
      : '';

    return {
      score,
      riskLevel,
      commuteTimeMin: finalDurationMin,
      distanceKm,
      heatIndex,
      hasSstAlert: heatIndex > 32,
      sstAlertText,
      hasWeatherAlert: rainProb > 50,
      weatherAlertText,
      hasTrafficAlert: delayMin > 15,
      trafficAlertText,
      routeDelayMin: delayMin,
      factors,
      temp,
      rainProb,
      posto: geo.posto,
  ...
  // Unified Fast Mobile Tab Navigation Handler
  lastMobileTabChangeTime = 0;

  changeMobileTab(tab: 'portal' | 'escala' | 'perfil' | 'equipe' | 'indicadores', event: Event) {
    if (event) {
      try {
        event.preventDefault();
        event.stopPropagation();
      } catch (e) {
        // Safe-guard for standard Event compatibility
      }
    }
    const now = Date.now();
    if (now - this.lastMobileTabChangeTime < 100) {
      return; // prevent duplicate clicks / touches
    }
    this.lastMobileTabChangeTime = now;
    
    // Switch the tab instantly via Signal
    this.activeSubTab.set(tab);
    
    // Smoothly scroll the container back to top when switching tabs
    setTimeout(() => {
      const container = document.getElementById('portal_tab_content');
      if (container) {
