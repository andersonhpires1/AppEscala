...
// Exclude standard void/self-closing tags
const selfClosingTags = ['img', 'input', 'br', 'hr', 'meta', 'link'];
// Exclude inline elements that are closed inline or not strict for block layouts
const inlineTags = ['span', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'button', 'a', 'strong', 'b', 'i', 'svg', 'path', 'defs', 'line', 'circle', 'text', 'rect', 'g', 'option', 'select', 'label'];

for (let i = 466; i < 4407; i++) {
  const line = lines[i];
  if (!line) continue;
  const lineNum = i + 1;
        ...
        if (tagName && !selfClosingTags.includes(tagName) && !inlineTags.includes(tagName)) {
          if (htmlStack.length > 0) {
            ...
            if (popped.name !== tagName) {
              console.log(`[Line ${lineNum}] Mismatch: Closed </${tagName}>, expected </${popped.name}> (opened at line ${popped.line})`);
              htmlStack.push(popped); // put back
            }
          } else {
...
console.log('--- Unclosed Tags Inside Section ---');
htmlStack.forEach(t => {
